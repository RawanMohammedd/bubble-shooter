...
void Filling_upper_balls_level3(Texture& mint, Texture& purple, Texture& light, Texture& yelow, Texture& orangee, Sprite upper_ball_level3[18][25])
{
	for (int row = 0; row < 11; row++)
	{
		...
		if (row == 5)
		{
			...
			upper_ball_level3[row][9].setPosition(49 * 9 , 330); 
			...
			if (ballss.shooting_ball[counter3].getTexture() == &mint && upper_ball_level3[4][i].getTexture() == &mint)
			{
				...
				if (i >= 12)
				{
					for (int r = 4; r < 18; r++)
					{
						...
						{
							upper_ball_level3[r][c].setScale(0, 0);
							/*upper_ball_level3[4][11].setScale(0, 0);
							upper_ball_level3[4][10].setScale(0, 0);*/
						}
					}
					for (int r = 4; r < 18; r++)
					{
...
int main()
{
	...
	hoome.loadFromFile("111.jpg");

	//sprites

	Sprite backs, base, cannon_sprite,upper_ball_colors[10][25], back, bub, st[2],
	...
	for (int j = 0; j < 2;j++)
	{
		st[j].setTexture(start);
		st[j].setScale(0.7, 0.7);
	}
	st[0].setPosition(270, 410);
	st[1].setPosition(520, 410);
	...
	homeback.setTexture(hoome);
	homeback.setPosition(420, 400);
	homeback.setScale(0.75, 0.5);
	...
	Text sstart, play, exit, totalscores, hs, ssound,backh, leve, score, text, next, hello,
		...
		level2 , level2game , playgain2 , level3game , home , credit , ournames , backname;

	sstart.setFont(font);
	sstart.setString("START");
	sstart.setPosition(300, 405);
	...
	home.setFont(font);
	home.setString("HOME");
	home.setPosition(440,390);
	home.setCharacterSize(60);

	credit.setFont(font);
	credit.setString("Credits");
	credit.setPosition(540,405);
	credit.setCharacterSize(65);
	credit.setFillColor(Color(90, 60, 80, 255));

	ournames.setFont(fffont);
	ournames.setString("\t\t\t\tPROGRAMMED BY : \n\n\t\t\t\t RANEEM MOHAMED \n\t\t\t\t RAWAN MOHAMED \n\t\t\t\t DANA AHMED \n\t\t\t\t NADA AHMED \n\t\t\t\t SHAMS MAGDY \n\t\t\t\t MANAR MOHAMED");
	ournames.setCharacterSize(40);
	ournames.setPosition(170 , 200);
	ournames.setFillColor(Color::Black);

	backname.setFont(font);
	backname.setString("BACK");
	backname.setPosition(580, 680);
	backname.setCharacterSize(80);
	backname.setFillColor(Color(90, 60, 80, 255));
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (event.KeyPressed)
			{
				...
				if (point.getGlobalBounds().intersects(home.getGlobalBounds()))
				{
					home.setFillColor(Color(0, 0, 0, 255));
				}
				else
					home.setFillColor(Color(90, 60, 80, 255));


				if (point.getGlobalBounds().intersects(credit.getGlobalBounds()))
				{
					credit.setFillColor(Color(0, 0, 0, 255));
				}
				else
					credit.setFillColor(Color(90, 60, 80, 255));

				if (point.getGlobalBounds().intersects(backname.getGlobalBounds()))
				{
					backname.setFillColor(Color(0, 0, 0, 255));
				}
				else
					backname.setFillColor(Color(90, 60, 80, 255));
			...
			if (Mouse::isButtonPressed(Mouse::Left))
			{
				...
				if (count == 0)
				{
					...
					if
						(point.getGlobalBounds().intersects(credit.getGlobalBounds()))
					{
						sound.play();
						count = 15;
					}
					...
					if (point.getGlobalBounds().intersects(playagain.getGlobalBounds()))
					{
						...
						for (int r = 0;r < 9;r++)
						{
							if (r == 0)
							{
								for (int c = 11;c < 25;c++)
								{
									if (upper_ball_level3[r][c].getScale().x > 0 && upper_ball_level3[r][c].getScale().y > 0)
									{
										upper_ball_level3[r][c].setScale(0, 0);
									}
								}
